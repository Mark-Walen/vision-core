...
template<typename Tp>
cv::Mat vectorToMat(const std::vector<Tp>& vec, unsigned int rows, unsigned int cols) {
    if (rows * cols != vec.size()) {
        throw std::invalid_argument("Matrix dimensions do not match the vector size.");
    }

    return cv::Mat(rows, cols, cv::DataType<Tp>::type, const_cast<Tp*>(vec.data())).clone();
}
