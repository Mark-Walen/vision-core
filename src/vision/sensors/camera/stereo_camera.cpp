...
namespace vlue::sensors {
    StereoCamera::StereoCamera(const Camera &left, const Camera &right, const _r_type &R_,
                               const _t_type &T_, const _q_type &Q_)
        : l(left), r(right), R(R_.clone()), T(T_.clone()), Q(Q_.clone()),
          size(static_cast<int>(left.width), static_cast<int>(left.height)) {
    ...
    StereoCamera::StereoCamera(const std::string &camera_config_path) {
        ...
        R = YAMLUtils::yamlNodeToMat(config["R"], 3, 3, CV_64F, MatFillType::Eyes);
        T = YAMLUtils::yamlNodeToMat(config["tvec"], 3, 1, CV_64F);
        Q = YAMLUtils::yamlNodeToMat(config["Q"], 4, 4, CV_64F, MatFillType::Eyes);
    }
